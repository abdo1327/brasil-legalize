...
export default function ClientsListPage() {
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      if (data.success) {
        // API returns { success, items } - normalize to clients
        const clientList = data.items || data.clients || [];
        setClients(clientList.map((c: any) => ({
          ...c,
          db_id: c.id, // Database ID (integer)
          id: c.client_id || c.id, // Display ID (CLT-xxxx)
          tags: Array.isArray(c.tags) ? c.tags : [],
          cases_count: parseInt(c.cases_count) || 0,
          financial: {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/admin/dashboard/clients/import"
